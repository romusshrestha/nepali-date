...
export default function DateDisplay() {
  ...
  return (
    <>
      ...
      {/* main card */}
      <Card
        className={`w-80  shadow-lg overflow-hidden mx-2 my-2 rounded-lg ${isHoliday ? "bg-red-50" : "bg-gradient-to-r from-emerald-500 to-green-600"}`}>
        {/* card header */}
        <CardHeader
          className={` text-white ${isHoliday ? "bg-red-800" : "bg-gradient-to-r from-emerald-800 to-green-800"}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex-grow text-center">
              <CardTitle className="text-center font-bold text-2xl font-sans">
                {" "}
                {nepaliDate.nepaliMonth} {nepaliDate.singleDayData.n},{" "}
                {nepaliDate.bsDate[0]}
              </CardTitle>
              <p className="text-center text-sm font-sans">
                {nepaliDate.englishMonth.slice(0, 3)} {nepaliDate.enDate},{" "}
                {nepaliDate.enYear}
              </p>
              <p className="text-center text-sm mt-2 font-sans">
                {nepaliDate.singleDayData.t}
              </p>
              <p className="text-center text-lg mt-2 font-sans">
        ...
        {/* card content */}
        <CardContent
          className={`p-4 ${isHoliday ? "" : "bg-gradient-to-b from-emerald-50 to-green-100"}`}>
          ...
          {activeTab === "marriage" && (
            <Marrige isHoliday={isHoliday} nepaliDate={nepaliDate} />
          )}
        </CardContent>
      </Card>
    </>
